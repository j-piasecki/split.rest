...
export async function uncompleteSplitEntry(
  ...
  try {
    ...
    await client.query(
      'UPDATE split_participants SET pending = TRUE WHERE split_id = $1 AND user_id = $2',
      [args.splitId, args.userId]
    )
    ...
    if (args.userId === callerId) {
      ...
      notificationMessage = 'notification.completeSplit.payeeUncompletedEntry'
    } else {
      notificationTokens = await getNotificationTokens(client, args.userId)
      notificationMessage = 'notification.completeSplit.payerUncompletedEntry'
    }
